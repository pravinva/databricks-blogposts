...
from src.config import JUDGE_LLM_ENDPOINT, JUDGE_LLM_TEMPERATURE, JUDGE_LLM_MAX_TOKENS
from src.prompts_registry import get_prompts_registry
from src.validation.token_calculator import get_token_calculator
from src.validation.json_parser import get_json_parser
import time
import re
...
def validate_response(query: str, response: str, country: str = "AU", tools_used: list = None) -> Dict:
    """
    Convenience function for notebooks to validate responses.

    Args:
        query: User query
        response: Agent response to validate
        country: Country code (default: AU)
        tools_used: List of tools used in response (optional)
    ...
    if _global_validator is None:
        _global_validator = DeterministicValidator()

    result = _global_validator.validate(
