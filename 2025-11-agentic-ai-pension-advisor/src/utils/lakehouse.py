    ...
    try:
        ...
        if statement.status.state == StatementState.SUCCEEDED:
            ...
            if hasattr(statement.status, 'error') and statement.status.error:
                error_msg += f"\nError: {statement.status.error.message if hasattr(statement.status.error, 'message') else statement.status.error}"
            if hasattr(statement.status, 'error_message') and statement.status.error_message:
                error_msg += f"\nDetails: {statement.status.error_message}"
