...
from src.utils.lakehouse import get_workspace_client  # Use same workspace client as member cards
...
class EmbeddingCascadeClassifier:
    ...
    def __init__(self, 
        ...
        from src.utils.lakehouse import get_workspace_client
...
# Module-level instance for notebook convenience
_global_classifier = None

def classify_query(query: str, country: str = None, member_id: str = None) -> Dict:
    """
    Convenience function for notebooks to classify queries.

    Args:
        query: User query to classify
        country: Country code (optional, for context)
        member_id: Member ID (optional, for context)
    ...
    if _global_classifier is None:
        _global_classifier = EmbeddingCascadeClassifier()

    result = _global_classifier.classify(query)

    # Add requires_tools based on classification
    result['requires_tools'] = result.get('is_on_topic', False) and not result.get('off_topic', True)
    result['type'] = result.get('classification', 'unknown')
