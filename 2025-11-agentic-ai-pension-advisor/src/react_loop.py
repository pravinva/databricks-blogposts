...
from src.config import SQL_WAREHOUSE_ID, MAX_VALIDATION_ATTEMPTS, CLASSIFIER_LLM_ENDPOINT
from src.prompts_registry import get_prompts_registry
...
class ReactAgenticLoop:
    ...
    def finalize_response(self, response_text: str, state: AgentState) -> str:
        ...
        if not response_text:
            self.printf("⚠️ Warning: Empty response text in finalize_response")
        ...
        try:
            ...
            self.printf(f"⚠️ Error in finalize_response: {str(e)}")
            # Return the best response we have, even if personalization failed
    ...
    def run_agentic_loop(self, state: AgentState) -> Dict:
        ...
        try:
            from src.utils.progress import mark_phase_running, mark_phase_complete
        ...
        for attempt in range(1, MAX_VALIDATION_ATTEMPTS + 1):
            ...
            try:
                ...
                if not response_text:
                    self.printf(f"❌ Synthesis failed on attempt {attempt}: No response text generated")
                    if attempt < MAX_VALIDATION_ATTEMPTS:
                        continue
                    else:
                        # Last attempt failed - return error
                        raise ValueError("Failed to generate valid response after all attempts")

            except Exception as e:
                self.printf(f"❌ Synthesis error on attempt {attempt}: {str(e)}")
                if attempt < MAX_VALIDATION_ATTEMPTS:
                    continue
                else:
                    # Last attempt failed - return error
